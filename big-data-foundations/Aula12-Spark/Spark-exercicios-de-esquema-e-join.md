...
scala> val alunosDF = spark.read.csv("/user/aluno/marcia-castagna/data/escola/alunos.csv")
...
# Incluir o cabeçalho: option("header", "true")
# O padrão é não ter o cabeçalho
val alunosDF = spark.read.option("header", "true").csv("/user/aluno/marcia-castagna/data/escola/alunos.csv")

scala> val alunosDF = spark.read.option("header", "true").csv("/user/aluno/marcia-castagna/data/escola/alunos.csv")

	alunosDF: org.apache.spark.sql.DataFrame = [id_discente: string, nome: string ... 5 more fields]


# Leitura sem cabeçalho
spark.read.csv("/user/aluno/marcia-castagna/data/escola/alunos.csv").show(3)

	// Está lendo a primeira linha como registro
	+-----------+--------------------+------------+----------------+-----+-----------------+--------+
...
scala> val alunosDF = spark.read.option("header", "true").option("inferSchema", "true").csv("/user/aluno/marcia-castagna/data/escola/alunos.csv")
...
# Sempre vai "inferir para cima", evitando assim, que o tipo não comporte o dado
# Por exemplo: string e não char, double e não float
scala> alunosDF.printSchema
...
scala> alunosDF.write.saveAsTable("marcia.tab_alunos")

// Para validar:
// Ir no hdfs e olhar o diretório do warehouse OU 
// Acessar o beeline (hive-server) e verificar se a tabela existe
...
scala> val cursosDF = spark.read.option("header", "true").option("inferSchema", "true").csv("/user/aluno/marcia-castagna/data/escola/cursos.csv")
